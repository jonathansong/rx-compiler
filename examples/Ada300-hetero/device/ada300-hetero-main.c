...
int main(void)
{
    ...
    printf("[hetero-dev] Polling ivshmem at 0x%llx for matmul commands...\n",
    ...
    for (;;) {
        ...
        /* ----- Allocate output buffer for C[M×N] ------------------------- */
        ...
        /* ----- Execute MatMul on the Ada300 SNPU (RXOPS_ADA300) ---------- */
        uint64_t t0 = rdtime();
        int rc = rxops_bridge_matmul_f32(C, A, B, (int64_t)M, (int64_t)N, (int64_t)K);
        uint64_t t1 = rdtime();

        uint64_t ticks = t1 - t0;
        /* CLINT on ada300s_evk defaults to 10 MHz (ADA300S_EVK_ACLINT_DEFAULT_TIMEBASE_FREQ
         * is 25 MHz in ada300s_evk.h but the actual CLINT timebase used by
         * riscv_aclint_mtimer_create is ADA300S_EVK_ACLINT_DEFAULT_TIMEBASE_FREQ).
         * Print raw ticks; host can interpret. */
        printf("[hetero-dev]   MatMul done in %llu ticks, rc=%d\n",
               (unsigned long long)ticks, rc);

        /* Print first element of C as a sanity check. */
        if (rc == 0 && M > 0 && N > 0) {
        ...
        /* ----- Write result and signal the host -------------------------- */
